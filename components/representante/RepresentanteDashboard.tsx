...
export function RepresentanteDashboard() {
  ...
  // Solo residentes sin rol especial son candidatos a añadir como personal
  ...
  async function quitarPersonal(userId: string) {
    ...
    try {
      ...
      void residentesQuery.refetch();
      void personalQuery.refetch();
      setMensaje('Rol removido correctamente');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'No se pudo remover el rol');
    } finally {
      setQuitando(null);
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            ...
            <Card key={label}>
              <CardContent className="flex items-center justify-between p-5">
                <div><p className="text-sm text-muted-foreground font-medium">{label}</p><p className={`text-3xl font-bold ${color}`}>{value}</p></div>
                {icon}
              </CardContent>
            </Card>
          ))}
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div><p className="text-sm text-muted-foreground font-medium">Recaudado</p><p className="text-3xl font-bold text-amber-600">${recaudado.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</p></div>
              <DollarSign className="h-8 w-8 text-amber-600" />
            </CardContent>
          </Card>
        </div>

        {/* Avance cobranza + cuotas */}
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant={tab === 'personal' ? 'default'     : 'outline'} onClick={() => setTab('personal')}>Personal ({personal.length})</Button>
        </div>
              ...
              {personal.length === 0 ? (
                ...
                personal.map((p) => (
                  <div key={p.id} className="flex items-center justify-between rounded-xl border p-4">
                    ...
                    >
                      {quitando === p.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4 mr-1" />}
                      Quitar
                    </Button>
                  </div>
                ))
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {r.tenencia === 'inquilino' && r.nombrePropietario && (
                          <p className="text-xs text-amber-700">Inquilino · Dueño: {r.nombrePropietario}{r.telefonoPropietario ? ` · ${r.telefonoPropietario}` : ''}</p>
                        )}
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold">Añadir personal</h2>
                <p className="text-sm text-muted-foreground">Circuito {circuito?.nombre}</p>
              </div>
              <Button size="icon" variant="ghost" onClick={() => setModalPersonal(false)}><X className="h-4 w-4" /></Button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium">Residente del circuito</label>
                <select
                ...
                {candidatos.length === 0 && (
                  <p className="mt-1 text-xs text-amber-600">No hay residentes disponibles en tu circuito.</p>
                )}
              </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <Button variant="outline" onClick={() => setModalPersonal(false)} disabled={guardandoPersonal}>Cancelar</Button>
              <Button onClick={agregarPersonal} disabled={!residenteSelId || guardandoPersonal}>
                {guardandoPersonal ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UserPlus className="mr-2 h-4 w-4" />}
                {guardandoPersonal ? 'Guardando...' : 'Asignar'}
              </Button>
            </div>
          </div>
        </div>
