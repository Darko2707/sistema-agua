...
- Name: residente-dashboard.spec.ts >> Login >> precarga el usuario recordado y permite eliminarlo
- Location: tests\e2e\residente-dashboard.spec.ts:32:7
...
Locator: getByLabel('Correo electrónico')
Expected: "recordado@ejemplo.com"
Timeout: 5000ms
...
- main:
  ...
  - paragraph: Comprobando tu sesión
  - status: Preparando el acceso...
- alert
```
...
> 40  |     await expect(page.getByLabel('Correo electrónico')).toHaveValue('recordado@ejemplo.com');
  ...
  127 |       if (req.url().includes('mercadopago.com')) {
  128 |         redirectedTo = req.url();
  129 |       }
  130 |     });
  131 | 
  132 |     await pagarBtn.click();
  133 |     // Button goes into loading state
  134 |     await expect(page.getByRole('button', { name: /Redirigiendo/i })).toBeVisible({ timeout: 3_000 });
  135 |     await expect.poll(() => redirectedTo).toContain('mercadopago.com');
  136 |   });
  137 | });
  138 | 
  139 | test.describe('Resultado de pago post-MP', () => {
